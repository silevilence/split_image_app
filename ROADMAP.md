...
#### ✅ Checklist
- [ ] 将侧边栏分为 "Settings Area" 和 "Preview/Export Area"
- [ ] 中间增加可拖拽的分割线 (Draggable Splitter)
- [ ] 各区域独立滚动条 (当内容高度超出区域限制时)
- [ ] 记忆用户调整的分割位置 (持久化到配置)
...
lib/
├── widgets/
│   ├── resizable_panel.dart      # 可调整大小的面板组件
│   └── preview_panel.dart        # 更新: 拆分为独立区域
└── services/
    └── config_service.dart       # 更新: 添加面板尺寸配置
```
