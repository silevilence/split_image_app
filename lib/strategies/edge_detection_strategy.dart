...
/// 边缘检测策略
///
/// 基于 Sobel 边缘检测算法自动识别贴纸边界。
/// 通过检测图片中的边缘，找到边缘密度最低的区域作为分割线位置。
class EdgeDetectionStrategy extends GridGeneratorStrategy {
  /// 边缘强度阈值 (0-255)
  /// 高于此值的像素被认为是边缘
  ...
  /// 是否使用高斯模糊预处理
  final bool useGaussianBlur;

  EdgeDetectionStrategy({
    this.edgeThreshold = 50,
    this.valleyThreshold = 0.4,
  ...
  @override
  GridAlgorithmType get type => GridAlgorithmType.edgeDetection;
  ...
  Future<GridGeneratorResult> generate(GridGeneratorInput input) async {
    debugPrint('[EdgeDetection] generate() called');
    debugPrint('[EdgeDetection] hasPixelData: ${input.hasPixelData}');
    debugPrint(
      '[EdgeDetection] imageSize: ${input.imageWidth}x${input.imageHeight}',
    );
    debugPrint('[EdgeDetection] effectiveRect: ${input.effectiveRect}');
    debugPrint(
      '[EdgeDetection] target: ${input.targetRows} rows x ${input.targetCols} cols',
    );

    if (!input.hasPixelData) {
      debugPrint('[EdgeDetection] ERROR: No pixel data!');
      return GridGeneratorResult.failure('边缘检测算法需要像素数据');
    }

    debugPrint('[EdgeDetection] pixelData length: ${input.pixelData!.length}');

    // 在 Isolate 中运行计算密集型任务
    final result = await compute(
      _computeEdgeDetection,
      _EdgeDetectionInput(
...
class _EdgeDetectionInput {
  ...
  _EdgeDetectionInput({
    ...
    required this.useGaussianBlur,
  });
}

/// 在 Isolate 中执行边缘检测计算
GridGeneratorResult _computeEdgeDetection(_EdgeDetectionInput input) {
  try {
    // 1. 将图像转换为灰度图
    final grayscale = _convertToGrayscale(
      input.pixelData,
      input.imageWidth,
      input.imageHeight,
      input.effectiveLeft,
    ...
    // 2. 可选：高斯模糊预处理（减少噪声）
    final smoothed = input.useGaussianBlur
        ? _applyGaussianBlur(
            grayscale,
            input.effectiveWidth,
            input.effectiveHeight,
    ...
    // 3. 使用 Sobel 算子进行边缘检测
    final edges = _applySobelOperator(
      smoothed,
      input.effectiveWidth,
      input.effectiveHeight,
      input.edgeThreshold,
    ...
    // 4. 计算水平边缘密度投影（用于检测水平分割线）
    final horizontalProjection = _computeHorizontalEdgeProjection(
      edges,
      input.effectiveWidth,
      input.effectiveHeight,
    );

    // 5. 计算垂直边缘密度投影（用于检测垂直分割线）
    final verticalProjection = _computeVerticalEdgeProjection(
      edges,
      input.effectiveWidth,
      input.effectiveHeight,
    );

    // 6. 检测水平方向的波谷（边缘密度低的区域）
    ...
    final message =
        '边缘检测完成: 检测到 ${horizontalValleys.length} 个水平低边缘区域, '
        '${verticalValleys.length} 个垂直低边缘区域'
    ...
    return GridGeneratorResult.failure('边缘检测失败: $e');
  }
}

/// 将图像转换为灰度图
List<int> _convertToGrayscale(
  Uint8List pixelData,
  int imageWidth,
  int imageHeight,
  int effectiveLeft,
  ...
  final grayscale = List<int>.filled(effectiveWidth * effectiveHeight, 0);
  ...
  for (int y = 0; y < effectiveHeight; y++) {
    ...
    for (int x = 0; x < effectiveWidth; x++) {
      ...
      if (pixelIndex + 3 < pixelData.length) {
        ...
        // 使用加权平均计算灰度值（考虑 alpha）
        // 如果透明，视为背景色（白色）
        if (alpha < 128) {
          grayscale[y * effectiveWidth + x] = 255;
        } else {
          // ITU-R BT.601 标准加权
          final gray = (0.299 * r + 0.587 * g + 0.114 * b).round();
          grayscale[y * effectiveWidth + x] = gray.clamp(0, 255);
        }
      }
    }
  }
...
/// 应用 3x3 高斯模糊
List<int> _applyGaussianBlur(List<int> grayscale, int width, int height) {
    ...
    for (int x = 1; x < width - 1; x++) {
      ...
      for (int ky = -1; ky <= 1; ky++) {
        for (int kx = -1; kx <= 1; kx++) {
          final pixelValue = grayscale[(y + ky) * width + (x + kx)];
          sum += pixelValue * kernel[ky + 1][kx + 1];
        }
      }
      result[y * width + x] = (sum / kernelSum).round().clamp(0, 255);
    }
  }

  // 复制边缘像素
  for (int x = 0; x < width; x++) {
    result[x] = grayscale[x]; // 顶边
    result[(height - 1) * width + x] =
        grayscale[(height - 1) * width + x]; // 底边
  }
  for (int y = 0; y < height; y++) {
    result[y * width] = grayscale[y * width]; // 左边
    result[y * width + width - 1] = grayscale[y * width + width - 1]; // 右边
  }
...
/// 计算水平边缘密度投影（每行的边缘强度总和）
/// 边缘密度低的行可能是分割线位置
List<double> _computeHorizontalEdgeProjection(
  List<int> edges,
  int width,
  int height,
) {
  final projection = List<double>.filled(height, 0);
  ...
  for (int y = 0; y < height; y++) {
    ...
    // 归一化到 0-1 范围
    projection[y] = sum / (width * 255);
  }

  return projection;
}

/// 计算垂直边缘密度投影（每列的边缘强度总和）
List<double> _computeVerticalEdgeProjection(
  List<int> edges,
  int width,
  int height,
) {
  final projection = List<double>.filled(width, 0);
